    ...
    readFile.onload = async function (e) {
        ...
        api.upload(formData, true, {'Content-Type': 'multipart/form-data'}).then((res) => {
            insertImage({
                ...
                'data-src': res.data.fullUrl, // 图片地址
            })
...
function getArticleDetail(id) {
    api.articleDetail({id}).then((res) => {
        for (let key in form.value) {
            if (key == 'label') {
                form.value[key] = res.data[key].split(',').map(Number)
            } else if (key == 'filename') {
                form.value[key] = res.data['url'].substring(9)
            } else if (key == 'preUrl') {
                form.value[key] = res.data['fullUrl']
            } else {
...
function save() {
    formRef.value.validate((valid, fields) => {
        if (valid) {
            ...
            // const html = xss.process(VueMarkdownEditor.vMdParser.themeConfig.markdownParser.render(json.content))

            // json.content = html.replace(/v-md-plugin-tip tip/g, 'v-md-plugin-tip tip dark:text-black')
