<template>
    ...
    <div ref="content" class="box">
            ...
            <el-table :data="list" border stripe :height="tabHeight" :header-cell-style="headerCellStyle" :cell-style="{'text-align': 'center'}">
                ...
                <el-table-column label="操作" width="200" fixed="right">
                    <template #default="{row}">
                        ...
                        <el-popconfirm title="确定要删除该分类吗?" @confirm="delHandle(row.id)">
                            <template #reference>
        ...
        <base-dialog ref="logDialog" :title="logTitle" @submit="logSubmit" @cancle="logCancle">
            <log ref="logBox" :data="classifyDetail" />
        </base-dialog>
    </div>
</template>
...
const getList = () => {
    ...
    api.classifyList(json).then((res) => {
        list.value = res.data
    })
}

// 弹框
let logTitle = ref('新增类型')
const logDialog = ref()
const logBox = ref()
const currentId = ref()
let classifyDetail = ref()
function logHandle(id) {
    if (id) {
        currentId.value = id
        logTitle.value = '编辑类型'
        api.classifyDetail({id}).then((res) => {
...
async function logSubmit() {
    ...
    if (logTitle.value == '新增类型') {
        api.classifyAdd(json).then((res) => {
            successDeal('新增成功')
            logCancle()
            getList()
        })
    } else {
        api.classifyEdit(json).then((res) => {
            successDeal('修改成功')
            logCancle()
            getList()
        })
