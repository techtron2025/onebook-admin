<template>
    <div ref="content" class="box">
            ...
            <el-table :data="list" border stripe :height="tabHeight" :header-cell-style="headerCellStyle" :cell-style="{'text-align': 'center'}">
                ...
                <el-table-column label="操作" width="200" fixed="right">
                    <template #default="{row}">
                        ...
                        <el-popconfirm title="确定要删除该标签吗?" @confirm="delHandle(row.id)">
                            <template #reference>
        ...
        <base-dialog ref="logDialog" :title="logTitle" @submit="logSubmit" @cancle="logCancle">
            <log ref="logBox" :data="labelDetail" />
        </base-dialog>
    </div>
</template>
...
const getList = () => {
    ...
    api.labelList(json).then((res) => {
        list.value = res.data
    })
}

// 弹框
let logTitle = ref('新增标签')
const logDialog = ref()
const logBox = ref()
const currentId = ref()
let labelDetail = ref()
...
async function logSubmit() {
    ...
    if (logTitle.value == '新增标签') {
        api.labelAdd(json).then((res) => {
            successDeal('新增成功')
            logCancle()
            getList()
        })
    } else {
        api.labelEdit(json).then((res) => {
            successDeal('修改成功')
            logCancle()
            getList()
        })
